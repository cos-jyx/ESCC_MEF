...
# from tensorflow.keras.layers.convolutional import Conv3D, AveragePooling3D,MaxPooling3D
...
def _shortcut3d(input, residual):
    """3D shortcut to match input and residual and merges them with "sum"."""## ceil() 函数返回数字的上入整数
    stride_dim1 = ceil(input.shape[DIM1_AXIS] \
        / residual.shape[DIM1_AXIS])#_keras_shape
...
def _get_block(identifier):
    if isinstance(identifier, six.string_types):#来判断一个对象是否是一个已知的类型，类似 type()
                                                #type() 不会认为子类是一种父类类型，不考虑继承关系;isinstance() 会认为子类是一种父类类型，考虑继承关系。
                                                #>>>a = 2 # >>> isinstance (a,int) # True

        res = globals().get(identifier)         #globals以字典类型返回当前位置的全部全局变量
        if not res:
...
class Resnet3DBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions, reg_factor):
        ...
        block = pool1
        filters = 64
        for i, r in enumerate(repetitions):#enumerate(sequence, [start=0]),功能：将可循环序列sequence以start开始分别列出序列数据和数据下标
        ...
        dense11=Dropout(0.5)(dense1)##原0.3
        dense2=Dense(units=500,activation='relu')(dense11)
        dense22=Dropout(0.5)(dense2)##原0.25
